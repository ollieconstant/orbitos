...
import CookieBanner, { useCookieConsent } from './components/CookieBanner.jsx'
...
export default function App() {
  const { user } = useAuth()
  const isPro = false
  const { consent, accept, decline, hasChosen } = useCookieConsent()
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:'#030810', fontFamily:M, overflow:'hidden' }}>
      ...
      {!hasChosen && <CookieBanner onAccept={accept} onDecline={decline} />}
    </div>
  )
}

// Cookie banner is imported and shown in App
// Add this to the bottom of the App return statement before the closing div
