...
import { SidebarAd, UpgradePrompt } from './AdBanner.jsx'

const CAT_COLORS = { stations:'#34d399', starlink:'#94a3b8', weather:'#38bdf8', science:'#a78bfa', nav:'#fbbf24', debris:'#f87171', comms:'#fb923c' }
const CAT_LABELS = { stations:'Stations', starlink:'Starlink', weather:'Weather', science:'Science', nav:'GPS / Nav', debris:'Debris', comms:'Comms' }
...
export default function Sidebar({ satellites=[], risks=[], selected, onSelect, isPro=false }) {
  ...
  const filtered  = useMemo(() => satellites.filter(s => !query || s.name.toLowerCase().includes(query.toLowerCase()) || String(s.id).includes(query)), [satellites,query])
    ...
    <div style={{ width:260, height:'100%', display:'flex', flexDirection:'column', background:'#0f172a', borderRight:'1px solid rgba(255,255,255,0.07)', fontFamily:M, flexShrink:0 }}>
      <div style={{ padding:'10px 12px', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
        <div style={{ position:'relative' }}>
          ...
          <input style={{ width:'100%', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:7, padding:'7px 10px 7px 30px', fontSize:13, color:'#cbd5e1', outline:'none', fontFamily:M }} placeholder="Search satellites..." value={query} onChange={e=>setQuery(e.target.value)} />
        </div>
        <div style={{ fontSize:11, color:'rgba(148,163,184,0.35)', marginTop:5 }}>{filtered.length.toLocaleString()} of {satellites.length.toLocaleString()} satellites</div>
      </div>

      <div style={{ flex:1, overflowY:'auto' }}>
        {filtered.length===0 && <div style={{ padding:24, textAlign:'center', color:'rgba(148,163,184,0.35)', fontSize:13 }}>No satellites match "{query}"</div>}
        {filtered.slice(0,120).map(sat => {
          const isRisk=riskNames.has(sat.name), isSel=selected?.id===sat.id
          const col=isRisk?'#f87171':(CAT_COLORS[sat.cat]??'#38bdf8')
          const alt=sat.position?.alt??0
          return (
            <div key={sat.id} onClick={()=>onSelect(isSel?null:sat)} style={{ padding:'7px 14px', borderBottom:'1px solid rgba(255,255,255,0.04)', cursor:'pointer', display:'flex', alignItems:'center', gap:8, background:isSel?'rgba(56,189,248,0.07)':isRisk?'rgba(239,68,68,0.06)':'transparent', borderLeft:`2px solid ${isSel?'#38bdf8':isRisk?'#f87171':'transparent'}` }}>
        ...
        {filtered.length>120 && <div style={{ padding:'10px 14px', fontSize:12, color:'rgba(148,163,184,0.35)', textAlign:'center' }}>Showing 120 of {filtered.length.toLocaleString()} — search to narrow down</div>}
      </div>
        ...
        <div style={{ borderTop:'1px solid rgba(239,68,68,0.2)', background:'rgba(239,68,68,0.05)', flexShrink:0 }}>
          ...
          {risks.slice(0,4).map((r,i) => (
            <div key={i} style={{ padding:'7px 12px', borderBottom:'1px solid rgba(239,68,68,0.08)' }}>
              ...
              <div style={{ fontSize:11, color:'rgba(252,165,165,0.55)', marginTop:2 }}>{r.distanceKm} km · {r.severity}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {!isPro && <SidebarAd />}
      {!isPro && <UpgradePrompt onUpgrade={()=>window.location.href='/pricing'} />}
    </div>
  )
}
